...
def article_generator_core(subject, language, num_sections, max_token):
    ...
    intro_prompt = f"Write an introductory in '{language}' section for an article about '{subject}'."
    ...
    for _ in range(num_sections):
        ...
        next_section_prompt = f"Based on the following article content:\n\n{article}\n\nWhat is the next important aspect to cover in '{language}' about '{subject}'?"
        ...
        content_prompt = f"Discuss the following topic in '{language}' in the context of '{subject}': {next_topic}"
...
def article_generator_core_ui():
    ...
    if 'openai_api_key' not in st.session_state:
        ...
        st.stop()
    if not subject:
        st.info("Please write the subject of your article to continue.")
        st.stop()
    if not language:
        st.info("Please write the language of your article to continue.")
        st.stop()
    elif st.button("🚄 Perform Request"):
        st.info("Performing")
        st.session_state["response"] = article_generator_core(subject, language, number_of_sections, max_token)
